...
async def on_message(message):
    ...
    if message.channel.id == logsChannelID:
        ...
        if message.content.startswith(prefix):
            ...
            server_ip = _ba.our_ip
            server_port = _ba.our_port
            dc_serverid = message.guild.id
            ...
            # important settings for discordbot to control :D
                ...
                if len(m) == 3:
                    ...
                    if setting_name == 'joinclaim':
                        if settings['JoinClaim'] == (setting_value == 'true'):
                            await message.channel.send(f">>> <a:arrow:1178627121137074226>**The `{setting_name}` setting is already `{setting_value.capitalize()}` in {servers}**")
                            return
                        settings['JoinClaim'] = setting_value == 'true'
                        setting_desc = 'Join Claim'
                    elif setting_name == 'notify':
                        ...
                        await message.channel.send(f">>> <a:arrow:1178627121137074226>**Invalid setting name `{setting_name}`. Supported settings: joinclaim, notify, comptext, whitelistplayers, nightmode, allowdm**")
                        return

                    with open(setting_json_path, 'w') as file:
                         json.dump(settings, file, indent=4)
                    await message.channel.send(f">>> <a:arrow:1178627121137074226>**Successfully set `{setting_desc}` to `{setting_value.capitalize()}` in {servers}**")
                else:
                    await message.channel.send(">>> <a:arrow:1178627121137074226><a:wrong:1178796743681392681>**Invalid Format! Use `{0}{1}settings <setting> <On or Off>`**".format(prefix, cmd))
           ...
           #check last 15 complaints players :D
            ...
            #cmds guide :)
                ...
                await message.channel.send(f">>> ## __(To find out server code type {prefix}sc) __\n:arrow_right:**__CHANGE ANY STATS-DATA COMMANDz:__**\n:arrow_right:`{prefix}add{currency} (pbid) (count)` - Add {currency} to a player in all servers.\n:arrow_right:`{prefix}remove{currency} (pbid) (count)` - Remove {currency} from player in all servers.\n:arrow_right:`{prefix}(server-code)add{currency} (pbid) (count)` - Add {currency} to a player in a specific server.\n:arrow_right:`{prefix}(server-code)remove{currency} (pbid) (count)` - Remove {currency} from player in specific server.\n:arrow_right:`{prefix}(server-code)csd (pbid) (statsdata to change) (count)` - Change (statsdata) of a player in a specific server.\n:arrow_right:Use statsdata - `scores, kills, deaths, games, kd`")
           ...
           # add tickets to player in a specific server
                ...
                if len(m) == 3:
                    ...
                    if coin:
                        addcoins(ac, count)    
                        await message.channel.send(f">>> :arrow_right:**Successfully added {count} {currency} to PB ID `{ac}` in {servers}**")                    
                    else:
          ...
          # remove tickets to player in a specific server
                ...
                if len(m) == 3:
                    ...
                    if coin:
                        ...
                        await message.channel.send(f">>> :arrow_right:**Successfully Removed {count} {currency} from PB ID `{ac}` in {servers}**")                    
                    else:
                        await message.channel.send(f">>> :arrow_right:**User `{ac}` is not there in my bankdata in {servers}**")
                else:
            ...
            #effect count cmd :))))))
                ...
                if custom_data and 'customeffects' in custom_data:
                    ...
                    if effect_data:
                        ...
                        await message.channel.send(f">>>  :arrow_right:**`({effect_count})` Custom Effects in {servers}:\n{effect_list}**")
                    else:
